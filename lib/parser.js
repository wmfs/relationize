...
function schemaFilesParser (schemaFiles) {
  const audit = {
  ...
  audit.expectedDbSchemaNames = Object.keys(audit.schemas)

  // Loop over all expected schemaNames
  audit.expectedDbSchemaNames.forEach(dbSchemaName => {
    const schema = audit.schemas[dbSchemaName]
    schema.tables = {}

    // Go through all files and add any tables for this schema
    schemaFiles.forEach(candidateSchemaFile => {
...
function convertNamespaceToSchemaName (namespace) {
  return (namespace) ? _.kebabCase(namespace).replace(/-/g, '_') : DEFAULT_SCHEMA_NAME
}
...
module.exports = schemaFilesParser
